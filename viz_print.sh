...
for hash in ${StringArray[@]}; do
    echo $hash
    python3 ./src/visualize.py --input_path=reduced.lang --key=#$hash | head 
#    python3 ./src/visualize.py --input_path=reduced.country --key=#$hash 
done

echo -all
python3 ./src/visualize.py --input_path=reduced.lang --key=_all | head 
